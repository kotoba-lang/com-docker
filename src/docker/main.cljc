(ns docker.main
  "Kotodama WASM entrypoint for the Docker clean-room actor (L5) — Clojure port.
  ...
  facts (`docker.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "docker")
(def tier "L5")
...
(def entity-specs
  [{:entity "Container" :plural "containers" :id-prefix "docker_con"
    :fields [:image :imageID :command :state :created :sizeRw]
    :required [:image]
    :coerce {:created :int :sizeRw :int}
    :refs {}}
   {:entity "Image" :plural "images" :id-prefix "docker_ima"
    :fields [:parent :comment :container :size :created]
    :required [:parent]
    :coerce {:size :int}
    :refs {}}
   {:entity "Network" :plural "networks" :id-prefix "docker_net"
    :fields [:name :scope :driver :enableIPv6 :internal :attachable :created]
    :required [:name]
    :coerce {:enableIPv6 :bool :internal :bool :attachable :bool}
    :refs {}}
   {:entity "Volume" :plural "volumes" :id-prefix "docker_vol"
    :fields [:name :driver :mountpoint :scope]
...
(defn emit-facts
  "EAVT facts for one record: {\"docker.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "docker-compat" :tier tier :entities entities} 200])
...
(defn start! [] :docker-compat/ready)
